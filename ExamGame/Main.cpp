...
void Main() {
	...
	while (System::Update())
	{
		if (scene == 0) {
			...
			SimpleGUI::CheckBox(probfirst, U"Not Reverse?", Vec2(500, 135));
			...
			if (enterdown || (!pre_mouse && start.onclick())) {
				if (quest.size() == 0) {
					TextReader texts(U"Probs/Probs.txt");
					String str;
					while (texts.readLine(str)) {
						pnames << str;
					}
				}
				else {
		...
		loop:;
			...
			pFont(Format((int)Scene::Time() / 60) + U"' " + Format((int)Scene::Time() % 60) + U"''  has left").draw(50, 40);
			SimpleGUI::CheckBox(probfirst, U"Not Reverse?", Vec2(580, 5));
